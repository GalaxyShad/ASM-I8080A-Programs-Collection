# Task #5 - "Resource management of the UMPK-80"

## Purpose
Study the ways of controlling the stand resources at the system software level.

## Task
Pressing the key "4" causes the lighted segments to rotate clockwise, and pressing the key "6" causes them to rotate counterclockwise. The rotation speed is controlled by the keys: "2" - decrease, "9" - increase (across the display)

## Solution Description
The software obtained in the course of the laboratory work performs the output of the animation specified by the user with the help of an array of frames, which are located starting from address 0900.  
Each frame consists of 6 values (1 value for each segment).
The frame array is bounded by the numbers 44 and FF.
The program copies frames one by one, with speed and direction adjustable during execution, into the arguments area of the function "Single scan of the display" and draws them.
